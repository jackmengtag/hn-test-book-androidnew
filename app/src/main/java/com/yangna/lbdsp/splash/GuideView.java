...
public class GuideView extends View implements ViewPager.OnPageChangeListener {
    ...
    private static final float DOT_Y_POSITION = 1.0f;
    ...
    public GuideView(Activity activity, List<SinglePage> guideContent, boolean drawDot, Bitmap dotDefault, Bitmap dotSelected) {
        ...
        if (guideContent != null && guideContent.size() > 1 && drawDot) {
            ...
            for (int i = 0; i < guideContent.size(); i++) {
                SingleElement e = new SingleElement(mDotXStart + mDotXPlus * i, mDotY - 100, mDotXStart + mDotXPlus * i, mDotY - 100, 1.0f, 1.0f, dotDefault);
                mDotList.add(e);
            }
            mSelectedDot = new SingleElement(mDotXStart + mDotXPlus * mPosition, mDotY - 100, mDotXStart + mDotXPlus * mPosition, mDotY - 100, 1.0f, 1.0f, dotSelected);
        }
    }
